/**
 * Lightweight WebSocket client to the FastAPI `/ws/notifications`
 * channel. Auto-reconnects with backoff. Used to push live notification
 * updates into TanStack Query caches and the toast viewport.
 */
...
export function connectNotifications(handlers: WsHandlers): () => void {
  ...
  const open = () => {
    ...
    if (!initData) {
      // Without auth there is no point reconnecting in a tight loop.
